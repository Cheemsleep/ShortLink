...
import com.cfl.shortlink.project.config.GotoDomainWhiteListConfiguration;
    ...
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        ...
        try {
            ...
            if (shortLinkDO.getValidDate() != null && shortLinkDO.getValidDate().before(new Date())) {
                log.error("创建的短链接: {} 是过期的短链接", fullShortUrl);
                throw new ServiceException("不允许生成过期短链接");
            }
        ...
        if (Objects.equals(hasShortLinkDO.getGid(), requestParam.getGid())) {
            ...
            try {
                ...
                LambdaUpdateWrapper<ShortLinkDO> deleteWrapper = Wrappers.lambdaUpdate(ShortLinkDO.class)
                        .eq(ShortLinkDO::getFullShortUrl, requestParam.getFullShortUrl())
                        .eq(ShortLinkDO::getGid, hasShortLinkDO.getGid())
                        .eq(ShortLinkDO::getDelTime, timeMillis)
                        .eq(ShortLinkDO::getDelFlag, 0)
                        .eq(ShortLinkDO::getEnableStatus, 0);
                ...
                LinkLocateStatsDO LinkLocateStatsDO = com.cfl.shortlink.project.dao.entity.LinkLocateStatsDO.builder()
                        .gid(requestParam.getGid())
                        .build();
                linkLocateStatsMapper.update(LinkLocateStatsDO, linkLocaleStatsUpdateWrapper);
                //操作系统统计访问重写gid
